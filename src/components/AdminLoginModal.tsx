...
import { User, KeyRound, AlertCircle, ShieldCheck, LogIn } from 'lucide-react';
...
export const AdminLoginModal: React.FC<AdminLoginModalProps> = ({ isOpen, onClose, onLoginSuccess, theme = 'light' }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Simulate a quick network delay for smoother UI feedback
      await new Promise((resolve) => setTimeout(resolve, 400));

      // Local client-side credential verification
      if (username === 'admin' && password === 'admin123') {
        ...
        localStorage.setItem('adminToken', mockAdminUser.token);
